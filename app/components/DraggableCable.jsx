...
export default function DraggableCable({
  ...
  const [start, setStart] = useState({ x: startX ?? initialX ?? 0, y: startY ?? initialY ?? 0 });
  const [end, setEnd] = useState({ x: endX ?? initialX + 100 ?? 100, y: endY ?? initialY + 100 ?? 100 });
  ...
  const { isAttacking, isPaused, isArranged, lastEndObject, setLastEndObject } = useAttack();

  const DDOS_ATTACK_FLOW = ["Attack PC", "Router", "Firewall", "Switch", "Server", "PC"];
  ...
  useEffect(() => {
    if (isArranged) {
      console.log(`[DEBUG] 0. isArranged: ${isArranged}, startX: ${startX}, startY: ${startY}, endX: ${endX}, endY: ${endY}`);
  ...
  useEffect(() => {
    ...
    if (isNaN(start.x) || isNaN(start.y) || isNaN(end.x) || isNaN(end.y)) {
      console.warn("[DEBUG] 1. Skipping findConnectedObject due to invalid coordinates.");
      return;
    }
  ...
  useEffect(() => {
    console.log(`[DEBUG] 2. isAttacking: ${isAttacking}, startObject: ${startObject}, endObject: ${endObject}`);
    ...
    if (isAttacking && startObject && endObject) {
      ...
      console.log(`[DEBUG] 2. startIdx: ${startIdx}, endIdx: ${endIdx}`);
        ...
        const isAttackPCPresent = (isAttackPCStart && !isAttackPCEnd)  || (!isAttackPCStart && isAttackPCEnd);

        // 흐름 이어짐 체크 (lastEndObject와 이어지는 경우)
        const isFlowContinuationFromStart = lastEndObject === startObject && DDOS_ATTACK_FLOW.includes(endObject);
        const isFlowContinuationFromEnd = lastEndObject === endObject && DDOS_ATTACK_FLOW.includes(startObject);
        ...
        // 유효성 체크
        const isValidFlow = (isAttackPCPresent || isFlowContinuationFromStart || isFlowContinuationFromEnd) &&
            flowStartIdx !== -1 && flowEndIdx !== -1;

        console.log(`[DEBUG] 2. isAttackPCPresent: ${isAttackPCPresent}, isFlowContinuationFromStart: ${isFlowContinuationFromStart}, isFlowContinuationFromEnd: ${isFlowContinuationFromEnd}, flowStartIdx: ${flowStartIdx}, flowEndIdx: ${flowEndIdx}, isValidFlow: ${isValidFlow}`);
      ...
      if (isValidFlow) {
          ...
          console.log(`[DEBUG] 2. cablesInPath: ${cablesInPath.map((cable) => `${cable.id}(${cable.order})`).join(", ")}`);

          // 경로 내 케이블에 순서 부여
          const totalSteps = Math.abs(flowEndIdx - flowStartIdx);
          const updatedCables = cablesInPath.map((cable, idx) => {
              const step = Math.floor((totalSteps / cablesInPath.length) * idx) + Math.min(flowStartIdx, flowEndIdx);
              return { ...cable, order: step };
          });

          console.log(`[DEBUG] 2. updatedCables: ${updatedCables.map((cable) => `${cable.id}(${cable.order})`).join(", ")}`);
          ...
          const activateNextCable = () => {
              if (currentIndexRef.current < cablesInPathRef.current.length) {
                  ...
                  console.log(`[DEBUG] 2. Activating cable ${cable.id} at step ${cable.order}`);
  ...
  }, [isAttacking, isPaused, startObject, endObject, droppedItems, lastEndObject, setLastEndObject]);
